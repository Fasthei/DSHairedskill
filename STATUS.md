...
- `src/`：仓库唯一的代码，一个 stdio MCP server，`src/tools/` 下含侦察（recon）、交互（interact）、消耗探测（consumption）、输出处理（output）、制品扫描（artifact_scan）、供应链（supply_chain）、A2A、MCP 探测（mcp_probe）、检测（detection）、载荷托管（payload_host）、防御扫描器（defensive_scanners）共 11 个工具模块、23 个工具。

## 已验证可用

- 单包仓库（无 pnpm workspace），Node `^22.19.0 || >=24.0.0`，TypeScript 5.9.3，vitest 3.2.4。
- `pnpm build` 通过。
